...
## 三、样本与向量（v3.7.0 起审稿时临时提取，审完即弃，不持久化）

以下三类指纹均为审稿时从正文临时提取的数据形态（原 style-samples.jsonl / sentence-patterns.jsonl / lexicon.json 三文件已合并精简，见第一节），仅 style_fingerprint.json 主档案持久化。
...
### 3.3 作者用词库（持久化于 style_fingerprint.json 的 qualitative 字段）
...
每章审稿生成该章的临时指纹，与 `.review-db/style_fingerprint.json` 主档案比对：
